...
##### Read In File #####
## use file.choose()
behavioral.file.path = choose.files(default = '*.txt',
                                    caption = "select behavioral file",
                                    multi = F,
                                    filters=Filters['txt',])
## read file
get.beh.file.contents <- function(filepath,df){
  con = file(filepath, "r")
  i=1
  while ( TRUE ) {
...
file.contents = get.beh.file.contents(behavioral.file.path,data.frame('text'=NA))
...
cat('\nfile contents migrated to list. removing original dataframe')
rm(file.contents)
cat('\n')
...
for(i in seq(2,length(beh.data$log.frames))){
